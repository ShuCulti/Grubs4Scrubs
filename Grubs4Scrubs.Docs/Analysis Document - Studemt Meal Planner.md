...
The purpose of this analysis is to break down the problem, figure out what already exists, define what the app needs to do, and map out the data model that'll support everything. It's basically the groundwork before I start building anything serious.

# **Project Description**

Grubs4Scrubs is a student meal planner web application. The frontend runs on React 19 with Vite and Tailwind CSS, while the backend is an ASP.NET Core Web API built on .NET 10 with a SQL Server database. The app lets students browse budget-friendly recipes, plan their meals for the week, and generate grocery lists so they don't overbuy or forget stuff.
...
| FR-07 | Users can filter recipes by category, budget, and prep time | Must | In Progress |
| FR-08 | Users can search recipes by keyword | Must | In Progress |
...
User is the central entity. Every user has an email, a hashed password, a username, and optionally a Google ID for OAuth login. Users own recipes and meal plans.

Recipe holds all the info about a single meal: title, description, prep and cook times, servings, an estimated budget, a category, tags, and nutritional data (calories, protein, fats, carbs as integers). Ingredients and instructions are stored as JSON strings inside the Recipe table because the school doesn't allow an ORM, so I'm keeping things manageable with raw ADO.NET queries. Each recipe can optionally belong to a user via UserId.
...
ShoppingItem (originally planned as GroceryItem) is now a standalone entity with its own table, full backend CRUD, and a working frontend page. Each item has a name, quantity (as a string), price, a checkbox for marking it off, and a UserId for future per-user filtering. The current implementation is a manual shopping list rather than auto-generated from meal plans, but the table and API are in place for that integration later.
...
Here's how everything connects: a User can have zero or many Recipes (one-to-many). A User can also have zero or many MealPlans and zero or many Favorites (both one-to-many). Each MealPlan contains one or many MealSlots (one-to-many), and each MealSlot references exactly one Recipe (many-to-one). A MealPlan has zero or many GroceryItems (one-to-many). A Favorite links back to exactly one Recipe (many-to-one).
...
This analysis covers everything I need to start building Grubs4Scrubs with a clear direction. The gaps in existing solutions are real, the requirements are scoped and prioritized, and the domain model gives me a solid foundation to build the database and API on.

The Definition of Done for this analysis phase is: all functional and non-functional requirements are documented, the domain model and ERD are complete, existing solutions have been researched and compared, and stakeholders are identified. With all of that in place, I can move into designing the architecture and starting implementation with confidence that I'm building the right thing.
